...
export class AuthService {
  private apiUrl = 'http://localhost:5300/user';
  ...
  async login(userLogin: userlogin, redirectCallback: () => void) {
    try {
      ...
      let response = await fetch('http://localhost:5300/user/login', {
  ...
  async redirect() {
    try {
      ...
      const data: any = await this.http.get('http://localhost:5300/user/check_user_details', {
        headers: {
...
//request reset password
  private requestResetPasswordUrl = 'http://localhost:5300/user/requestPassword';
  ...
  //reset password
  private resetPasswordUrl = 'http://localhost:5300/user';
